...
import { processSpreadsheetEntries } from "@/lib/generate-pass";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const dryRun = ["1", "true", "yes"].includes(
      (formData.get("dryRun") ?? request.nextUrl.searchParams.get("dryRun") ?? "")
        .toString()
        .toLowerCase(),
    );

    const buffer = Buffer.from(await file.arrayBuffer());
    const { entries, skipped } = parseSpreadsheetBuffer(buffer);
    ...
    console.info("[generate-cards-bulk] Processing entries", {
      fileName: file.name,
      total: entries.length,
      skipped: skipped.length,
    });

    const results = await processSpreadsheetEntries(entries);
    ...
    return jsonWithCors({
      ...
      message:
        failed === 0
          ? `Processed ${sent} event pass(es).`
          : `Processed ${sent} pass(es); ${failed} failed. See results for details.`,
    });
  } catch (error) {
    console.error("[generate-cards-bulk]", error);
